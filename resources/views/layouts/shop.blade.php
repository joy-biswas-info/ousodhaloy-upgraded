...
<head>
    ...
    {{-- Tailwind CSS (Play CDN – works without npm build step) --}}
    ...
    {{-- Custom component styles (on top of Tailwind) --}}
    <style>
        :root {
            --teal: #0e7673;
            --teal-dark: #0a5250;
...
<body class="bg-gray-50 font-inter">
    ...
    <div class="bg-teal-900 text-white text-xs py-1.5 hidden sm:block">
        <div class="max-w-7xl mx-auto px-4 flex justify-between items-center">
            <span><i class="fas fa-flag mr-1"></i>🇧🇩 Free delivery above
                ৳{{ \App\Models\Setting::get('free_delivery_min', 500) }} | Call:
                {{ \App\Models\Setting::get('site_phone', '09610016778') }}</span>
            <div class="flex gap-5">
                <a href="{{ route('track') }}" class="hover:text-teal-200 transition-colors"><i
                        class="fas fa-search-location mr-1"></i>Track Order</a>
                <a href="{{ route('checkout.index') }}?rx=1" class="hover:text-teal-200 transition-colors"><i
                        class="fas fa-file-medical mr-1"></i>Upload Prescription</a>
                @guest <a href="{{ route('auth.login') }}" class="hover:text-teal-200 transition-colors"><i c
                   lass="fas fa-sign-in-alt mr-1"></i>Login</a> @endguest
            </div>
        </div>
    </div>
    ...
    <header class="bg-teal-700 sticky top-0 z-50 shadow-md" x-data="{ userMenu: false }">
        <div class="max-w-7xl mx-auto px-4 h-16 flex items-center gap-4">
            ...
            {{-- Logo --}}
            <a href="{{ route('home') }}" class="flex items-center gap-2 flex-shrink-0 text-white">
                @if(\App\Models\Setting::get('site_logo'))
                    <img src="{{ asset('storage/' . \App\Models\Setting::get('site_logo')) }}" class="h-9 w-auto"
                ...
                @else
                    <div
                        ...
                        ও</div>
                    <span
                        class="font-bold text-xl tracking-tight hidden sm:block">{{ \App\Models\Setting::get('site_name', 'Ousodhaloy') }}</span>
                @endif
            </a>
            ...
            <div class="flex items-center gap-2 flex-shrink-0">
                ...
                <a href="{{ route('cart.index') }}"
                    ...
                    <span id="cart-count"
                        class="absolute -top-1.5 -right-1.5 bg-white text-orange-600 text-[10px] font-bold rounded-full w-5 h-5 flex items-center justify-center {{ \App\Http\Controllers\Shop\CartController::getCount() === 0 ? 'hidden' : '' }}">
                ...
                <div class="relative">
                    <button @click="userMenu = !userMenu"
                        ...
                        <span
                            class="hidden sm:inline">{{ auth()->user()?->name ? Str::words(auth()->user()->name, 1, '') : 'Login' }}</span>
        ...
        <div class="max-w-7xl mx-auto px-4">
            <div class="flex items-center gap-1 overflow-x-auto scrollbar-hide h-11">
                <a href="{{ route('shop.index') }}"
                    class="flex-shrink-0 text-xs font-medium text-gray-600 hover:text-teal-700 hover:bg-teal-50 px-3 py-2 rounded-lg transition-colors whitespace-nowrap {{ request()->routeIs('shop.index') && !request()->has('category') ? 'text-teal-700 bg-teal-50' : '' }}">
                    🏠 All
                </a>
                @foreach(\App\Models\Category::active()->take(12)->get() as $cat)
                    <a href="{{ route('shop.index', ['category' => $cat->slug]) }}"
                        class="flex-shrink-0 text-xs font-medium px-3 py-2 rounded-lg transition-colors whitespace-nowrap {{ request('category') === $cat->slug ? 'text-teal-700 bg-teal-50 font-semibold' : 'text-gray-600 hover:text-teal-700 hover:bg-teal-50' }}">
                        {{ $cat->icon }} {{ $cat->name }}
                    </a>
                @endforeach
            </div>
    ...
    <footer class="bg-gray-900 text-gray-400 pt-12 pb-6 mt-16">
        <div class="max-w-7xl mx-auto px-4">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-8 mb-8">
                <div class="col-span-2 md:col-span-1">
                    <div class="flex items-center gap-2 mb-3">
                        <div
                            ...
                            ও</div>
                        <span
                            class="text-white font-bold text-lg">{{ \App\Models\Setting::get('site_name', 'Ousodhaloy') }}</span>
                    </div>
                ...
                <div>
                    ...
                    <ul class="space-y-2 text-xs">
                        ...
                        <li><a href="{{ route('shop.index') }}" class="hover:text-white transition-colors">All
                                Products</a></li>
                        <li><a href="{{ route('shop.index', ['category' => 'medicine']) }}"
                                class="hover:text-white transition-colors">Medicine</a></li>
                        <li><a href="{{ route('track') }}" class="hover:text-white transition-colors">Track Order</a>
                        </li>
                        <li><a href="{{ route('auth.login') }}" class="hover:text-white transition-colors">My
                                Account</a></li>
                    </ul>
                </div>
                <div>
                ...
                <div>
                    ...
                    <ul class="space-y-2 text-xs">
                        <li class="flex items-center gap-2"><i class="fas fa-phone text-teal-500"></i>
                            {{ \App\Models\Setting::get('site_phone', '09610016778') }}</li>
                        <li class="flex items-center gap-2"><i class="fas fa-envelope text-teal-500"></i> {{
                            \App\Models\Setting::get('site_email','info@ousodhaloy.com') }}</li>
                        <li class="flex items-center gap-2"><i class="fas fa-map-marker-alt text-teal-500"></i>
                            {{ \App\Models\Setting::get('site_address', 'Dhaka, Bangladesh') }}</li>
            ...
            <div
                ...
                <p>© {{ date('Y') }} {{ \App\Models\Setting::get('site_name', 'Ousodhaloy') }} Ltd. All rights reserved.
                </p>
