    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div class="lg:col-span-2 space-y-3">
            ...
            <div class="bg-white rounded-xl border p-4 flex gap-4" id="cart-item-{{ $id }}">
                <a href="{{ route('shop.product', $item['product']->slug) }}" class="w-20 h-20 flex-shrink-0 bg-gray-50 rounded-xl border flex items-center justify-center overflow-hidden">
                ...
                <div class="flex-1 min-w-0">
                    <a href="{{ route('shop.product', $item['product']->slug) }}" class="font-semibold text-gray-800 text-sm hover:text-teal-700 line-clamp-2">{{ $item['name'] }}</a>
                    ...
                    <div class="flex items-center justify-between mt-2 flex-wrap gap-2">
                        <div>
                            ...
                            <span class="text-xs text-gray-400 line-through ml-1">৳{{ number_format($item['product']->mrp, 2) }}</span>
                            @endif
                        </div>
                        <div class="flex items-center gap-2">
                            <div class="flex items-center border rounded-lg overflow-hidden">
                                ...
                                <span class="px-3 py-1.5 text-sm font-bold min-w-[2.5rem] text-center">{{ $item['qty'] }}</span>
                                <form action="{{ route('cart.update', $id) }}" method="POST" class="inline">
                                    @csrf @method('PATCH')
                            ...
                            <span class="font-bold text-gray-800 text-sm">৳{{ number_format($item['subtotal'], 2) }}</span>
            ...
            <div class="bg-white rounded-xl border p-4">
                ...
                <div class="space-y-2 text-sm" id="summary">
                    ...
                    <p class="text-xs text-orange-600 bg-orange-50 rounded-lg px-3 py-2">
                        Add ৳{{ number_format(\App\Models\Setting::get('free_delivery_min', 500) - $totals['subtotal'], 2) }} more for FREE delivery!
                    </p>
                    @endif
                </div>
