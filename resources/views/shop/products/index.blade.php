            ...
            <div class="bg-white rounded-xl border p-4">
                ...
                <div class="space-y-1">
                    <a href="{{ route('shop.index', request()->except('category','page')) }}"
                        class="flex items-center justify-between px-2 py-1.5 rounded-lg text-xs transition-colors {{ !request('category') ? 'bg-teal-50 text-teal-700 font-semibold' : 'text-gray-600 hover:bg-gray-50' }}">
                        <span>All Products</span>
                    </a>
                    @foreach($categories as $cat)
                    <a href="{{ route('shop.index', array_merge(request()->except('page'), ['category' => $cat->slug])) }}"
                        class="flex items-center justify-between px-2 py-1.5 rounded-lg text-xs transition-colors {{ request('category') === $cat->slug ? 'bg-teal-50 text-teal-700 font-semibold' : 'text-gray-600 hover:bg-gray-50' }}">
