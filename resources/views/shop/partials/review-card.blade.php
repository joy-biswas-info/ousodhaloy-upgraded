                ...
                @auth
                    ...
                    @if (session('review_success'))
                        <div class="bg-green-50 border border-green-200 text-green-700 text-sm rounded-xl px-4 py-3 mb-4">
                            <i class="fas fa-check-circle mr-1.5"></i>{{ session('review_success') }}
                        </div>
                    @endif
                    <form method="POST" action="{{ route('shop.product.review', $product->slug) }}" class="space-y-4">
                        @csrf
                        <div>
                            <label class="text-xs font-semibold text-gray-600 block mb-2">Your Rating *</label>
                            <div class="flex gap-1" x-data="{ rating: {{ old('rating', 0) }}, hover: 0 }">
                                @for ($s = 1; $s <= 5; $s++)
                                    <button type="button" @click="rating = {{ $s }}"
                                        ...
                                        <i class="fas fa-star"
                                            :class="(hover || rating) >= {{ $s }} ? 'text-amber-400' :
                                                'text-gray-200'"></i>
                                    </button>
                                @endfor
