<div class="product-card">
    <a href="{{ route('shop.product', $product->slug) }}" class="block">
        ...
        {{-- @if ($product->express_delivery)
            <div class="badge-express bg-teal-100 rounded-2xl p-1">🚀 Express</div>
        @endif --}}
    ...
    <div class="card-body">
        ...
        <div class="flex justify-between">
            ...
            <button
                @if ($product->is_in_stock && !$product->requires_prescription) onclick="addToCart({{ $product->id }})"
            ...
            @else
                ...
                @else
                    <i class="fas fa-cart-plus text-[10px] mr-1"></i>Cart
                @endif
            </button>
        </div>
