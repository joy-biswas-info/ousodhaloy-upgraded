...
@section('breadcrumb', 'Orders / New Manual Order')

@section('content')
    <form method="POST" action="{{ route('admin.orders.manual-store') }}" id="manual-order-form" x-data="manualOrder()">
        @csrf
                    ...
                    <div class="mb-4">
                        ...
                        <select name="user_id" class="form-select" onchange="fillCustomer(this)">
                            ...
                            @foreach($customers as $c)
                                <option value="{{ $c->id }}" data-phone="{{ $c->phone }}" data-name="{{ $c->name }}">
                                    {{ $c->name }} — {{ $c->phone }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="bg-white rounded-xl border p-5 space-y-3">
                    ...
                    <div>
                        ...
                        <select name="payment_method" class="form-select">
                            @foreach(['cod' => 'Cash on Delivery', 'bkash' => 'bKash', 'nagad' => 'Nagad', 'ssl_commerz' => 'Card / Online', 'bank' => 'Bank Transfer'] as $val => $label)
