                    ...
                    <tbody>
                        @forelse($categories as $cat)
                            <tr x-data="{ editing: false }">
                                ...
                                <td>
                                    <span x-show="!editing"
                                        class="font-semibold text-sm text-gray-800">{{ $cat->name }}</span>
                                    <form x-show="editing" method="POST"
                                        action="{{ route('admin.categories.update', $cat) }}" class="flex gap-2">
                                        @csrf @method('PUT')
                                        <input type="text" name="name" value="{{ $cat->name }}" class="form-input"
                                            style="width:140px">
                                        <input type="text" name="icon" value="{{ $cat->icon }}" class="form-input"
                                        ...
                                        <input type="hidden" name="is_active"
                                            value="{{ $cat->is_active ? 'true' : 'false' }}">
                                ...
                                <td>
                                    <span
                                        class="text-xs font-semibold {{ $cat->is_active ? 'text-green-600' : 'text-gray-400' }}">
                                ...
                                <td>
                                    <div class="flex gap-1.5">
                                        ...
                                        <a href="{{ route('shop.index', ['category' => $cat->slug]) }}" target="_blank"
                                        ...
                                        <form method="POST" action="{{ route('admin.categories.destroy', $cat) }}"
                                            ...
                                            <button type="submit"
                                                ...
                                                {{ $cat->products_count > 0 ? 'disabled title=Has products' : '' }}>
                ...
                <div class="flex flex-wrap gap-1.5">
                    @foreach (['💊 Medicine', '🧪 Vitamins', '👶 Baby & Mother', '🩸 Diabetes', '❤️ Heart', '🧴 Skin Care', '👁️ Eye & Ear', '🦷 Dental', '💪 Fitness', '🌿 Herbal', '🩺 Devices', '🛁 Personal Care'] as $c)
                        <span class="text-xs bg-gray-100 px-2 py-1 rounded cursor-pointer hover:bg-teal-50"
                            onclick="document.querySelector('[name=name]').value='{{ explode(' ', trim($c), 2)[1] ?? $c }}'; document.querySelector('[name=icon]').value='{{ substr(trim($c), 0, 2) }}'">{{ $c }}</span>
                    @endforeach
                </div>
            </div>
        </div>
