                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="generic_name"
                                value="{{ old('generic_name', $product?->generic_name) }}" class="form-input"
                        ...
                        <div>
                            ...
                            <select name="category_id" class="form-select">
                                ...
                                @foreach ($categories as $cat)
                                    <option value="{{ $cat->id }}" @selected(old('category_id', $product?->category_id) == $cat->id)>{{ $cat->icon }}
                                        {{ $cat->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="number" id="price" name="price" step="0.01"
                                value="{{ old('price', $product?->price) }}" class="form-input" required
                                placeholder="10.80">
                        ...
                        <div>
                            ...
                            <input type="number" id="mrp" name="mrp" step="0.01"
                                value="{{ old('mrp', $product?->mrp) }}" class="form-input" placeholder="12.00">
                        ...
                        <div>
                            ...
                            <input type="number" id="discount_percent" name="discount_percent" step="0.01"
                                max="100" value="{{ old('discount_percent', $product?->discount_percent ?? 0) }}"
                        ...
                        <div>
                            ...
                            <select name="unit" class="form-select">
                                @foreach (['pcs', 'strip', 'bottle', 'box', 'pack', 'tube', 'vial', 'sachet', 'ml', 'mg', 'gm'] as $u)
                                    <option value="{{ $u }}" @selected(old('unit', $product?->unit ?? 'pcs') === $u)>{{ $u }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <input type="number" name="min_order_qty"
                                value="{{ old('min_order_qty', $product?->min_order_qty ?? 1) }}" class="form-input">
                        </div>
                        <div>
                            <label class="form-label">Max Order Qty</label>
                            <input type="number" name="max_order_qty"
                                value="{{ old('max_order_qty', $product?->max_order_qty ?? 100) }}" class="form-input">
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl border p-5">
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="form" value="{{ old('form', $product?->form) }}"
                                class="form-input" placeholder="Tablet, Syrup, Cream...">
                        </div>
                        <div>
                            <label class="form-label">Pack Size</label>
                            <input type="text" name="pack_size" value="{{ old('pack_size', $product?->pack_size) }}"
                                class="form-input" placeholder="10 tablets/strip">
                        </div>
                        <div class="col-span-3">
                            <label class="form-label">Tags (comma separated)</label>
                            <input type="text" name="tags"
                                value="{{ old('tags', $product ? implode(', ', $product->tags ?? []) : '') }}"
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="flash_sale_price" step="0.01"
                                value="{{ old('flash_sale_price', $product?->flash_sale_price) }}" class="form-input">
                        </div>
                        <div>
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <input type="text" name="meta_title"
                                value="{{ old('meta_title', $product?->meta_title) }}" class="form-input"
                                placeholder="Leave blank to use product name">
                        </div>
                        <div>
                            <label class="form-label">Meta Description</label>
                            <textarea name="meta_description" rows="2" class="form-input resize-none"
                                placeholder="Leave blank to auto-generate">{{ old('meta_description', $product?->meta_description) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-5">
                ...
                <div class="bg-white rounded-xl border p-5">
                    ...
                    <p class="text-xs text-gray-400 mt-2 text-center">
                        Upload images in <a href="{{ route('admin.media.index') }}" target="_blank"
                ...
                <div class="bg-white rounded-xl border p-5 space-y-3">
                    ...
                    @foreach ([['name' => 'is_active', 'label' => '✅ Active (visible in store)', 'default' => true], ['name' => 'is_featured', 'label' => '⭐ Featured on homepage', 'default' => false], ['name' => 'is_flash_sale', 'label' => '⚡ Flash Sale', 'default' => false, 'toggle' => 'isFlashSale'], ['name' => 'express_delivery', 'label' => '🚀 Express Delivery available', 'default' => false], ['name' => 'requires_prescription', 'label' => '💊 Prescription Required (Rx)', 'default' => false]] as $toggle)
