    ...
    <div class="space-y-5" x-data="{ tab: '{{ request('tab', 'general') }}' }">
        ...
        <div x-show="tab === 'orders'" class="bg-white rounded-xl border p-5">
            ...
            <form method="POST" action="{{ route('admin.settings.update') }}" class="space-y-4 mb-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div><label class="form-label">Default Delivery Charge (৳)</label><input type="number"
                            name="delivery_charge" value="{{ $settings['delivery_charge'] ?? 60 }}" class="form-input">
                    </div>
                    <div><label class="form-label">Free Delivery Above (৳)</label><input type="number"
                            name="free_delivery_min" value="{{ $settings['free_delivery_min'] ?? 500 }}"
                            class="form-input"></div>
                </div>
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="guest_checkout" value="true"
                        {{ ($settings['guest_checkout'] ?? '') === 'true' ? 'checked' : '' }} class="accent-teal-600">
            ...
            <div class="border-t pt-5">
                                ...
                                <tr x-data="{ editing: false }">
                                    ...
                                    <td>
                                        ...
                                        <form x-show="editing" method="POST"
                                            action="{{ route('admin.settings.delivery-zones.update', $zone) }}"
                                            ...
                                            <input type="number" name="delivery_charge"
                                                value="{{ $zone->delivery_charge }}" step="0.01"
                                                class="form-input py-1 text-xs" style="width:80px">
                                            <input type="number" name="free_delivery_above"
                                                value="{{ $zone->free_delivery_above }}" step="0.01"
                                                class="form-input py-1 text-xs" style="width:80px">
                                            <input type="number" name="estimated_days"
                                                value="{{ $zone->estimated_days }}" class="form-input py-1 text-xs"
                                                style="width:55px">
                                            <input type="hidden" name="is_active"
                                                value="{{ $zone->is_active ? '1' : '0' }}">
                                    ...
                                    <td><span
                                            class="text-xs font-semibold {{ $zone->is_active ? 'text-green-600' : 'text-gray-400' }}">{{ $zone->is_active ? 'Active' : 'Off' }}</span>
                                    </td>
                                    <td x-show="!editing">
                                        <div class="flex gap-1.5">
                                            ...
                                            <form method="POST"
                                                action="{{ route('admin.settings.delivery-zones.destroy', $zone) }}"
                ...
                <form method="POST" action="{{ route('admin.settings.delivery-zones.store') }}"
                ...
                <div class="overflow-x-auto">
                    <table class="admin-table">
                        ...
                        <tbody>
                            ...
                            @foreach (['Full Name', 'Phone Number', 'Division', 'District', 'Full Address'] as $alwaysField)
                                <tr class="bg-gray-50">
                                    <td class="text-sm font-medium text-gray-700">{{ $alwaysField }} <span
                            ...
                            @foreach ($fields as $f)
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <input type="checkbox" name="checkout_field[{{ $f['key'] }}][visible]"
                                            value="1" {{ $visible ? 'checked' : '' }} class="accent-teal-600">
                                    </td>
                                    <td class="text-center">
                                        <input type="checkbox" name="checkout_field[{{ $f['key'] }}][required]"
                                            value="1" {{ $required ? 'checked' : '' }} class="accent-teal-600">
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        <div>
                            ...
                            <input type="number" name="pathao_default_city_id"
                                value="{{ $settings['pathao_default_city_id'] ?? '' }}" class="form-input"
                        ...
                        <div>
                            ...
                            <input type="number" name="pathao_default_zone_id"
                                value="{{ $settings['pathao_default_zone_id'] ?? '' }}" class="form-input"
                                placeholder="Zone ID">
                        </div>
                        <div>
                            <label class="form-label">Default Area ID</label>
                            <input type="number" name="pathao_default_area_id"
                                value="{{ $settings['pathao_default_area_id'] ?? '' }}" class="form-input"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="steadfast_api_key"
                                value="{{ $settings['steadfast_api_key'] ?? '' }}" class="form-input font-mono"
                        ...
                        <div>
                            ...
                            <input type="password" name="steadfast_secret_key"
                                value="{{ $settings['steadfast_secret_key'] ?? '' }}" class="form-input"
            ...
            @if (!empty($settings['steadfast_api_key']))
                <div class="bg-white rounded-xl border p-5">
                    ...
                    <div class="flex items-center gap-3">
                        <div
                            class="flex items-center gap-2 {{ ($settings['steadfast_enabled'] ?? '') === 'true' ? 'bg-green-50 border-green-200 text-green-700' : 'bg-gray-50 border-gray-200 text-gray-500' }} border rounded-xl px-4 py-2.5">
                            <span
                                class="text-lg">{{ ($settings['steadfast_enabled'] ?? '') === 'true' ? '🟢' : '⚫' }}</span>
                            <div>
                                <p class="text-xs font-bold">Steadfast</p>
                                <p class="text-sm font-black">
                                    {{ ($settings['steadfast_enabled'] ?? '') === 'true' ? 'Enabled' : 'Disabled' }}</p>
                            </div>
                        </div>
                ...
                <form method="POST" action="{{ route('admin.settings.pixel') }}" class="space-y-5">
                    ...
                    <div class="p-4 bg-gray-50 rounded-xl">
                        ...
                        <div class="flex gap-3">
                            @foreach (['live' => ['🟢 Live Mode', 'bg-green-50 border-green-300 text-green-800'], 'test' => ['🧪 Test Mode', 'bg-yellow-50 border-yellow-300 text-yellow-800']] as $mode => [$mlabel, $mclass])
                                <label class="flex-1 cursor-pointer">
                                    <input type="radio" name="meta_pixel_mode" value="{{ $mode }}"
                                        {{ ($settings['meta_pixel_mode'] ?? 'test') === $mode ? 'checked' : '' }}
                                        class="sr-only peer">
                                    <div
                                        class="border-2 rounded-xl p-3 text-center text-sm font-bold transition-all peer-checked:border-teal-500 peer-checked:bg-teal-50 {{ $mclass }}">
                                        {{ $mlabel }}
                                    </div>
                                </label>
                            @endforeach
                        ...
                        <div class="mt-3" id="test-code-wrap" x-data
                            x-show="{{ ($settings['meta_pixel_mode'] ?? 'test') === 'test' ? 'true' : 'false' }}">
                            <label class="form-label">Test Event Code</label>
                            <input type="text" name="meta_pixel_test_event_code"
                                value="{{ $settings['meta_pixel_test_event_code'] ?? '' }}" class="form-input font-mono"
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="password" name="meta_access_token"
                                value="{{ $settings['meta_access_token'] ?? '' }}" class="form-input"
            ...
            @if (!empty($settings['meta_pixel_id']))
                <div class="bg-white rounded-xl border p-5">
                    ...
                    <div class="flex flex-wrap gap-3">
                        <div
                            class="flex items-center gap-2 bg-{{ ($settings['meta_pixel_mode'] ?? '') === 'live' ? 'green' : 'yellow' }}-50 border border-{{ ($settings['meta_pixel_mode'] ?? '') === 'live' ? 'green' : 'yellow' }}-200 rounded-xl px-4 py-2.5">
                            <span
                        ...
                        @if (!empty($settings['meta_pixel_test_event_code']) && ($settings['meta_pixel_mode'] ?? '') === 'test')
                            <div
                                ...
                                <div>
                                    ...
                                    <p class="text-sm font-mono font-black">{{ $settings['meta_pixel_test_event_code'] }}
                                    </p>
                                </div>
                            </div>
                        @endif
