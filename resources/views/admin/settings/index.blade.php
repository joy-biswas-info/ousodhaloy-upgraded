...
<div class="space-y-5" x-data="{ tab: '{{ request('tab','general') }}' }">
    ...
    <div x-show="tab === 'general'" class="bg-white rounded-xl border p-5">
        ...
        <form method="POST" action="{{ route('admin.settings.update') }}" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div><label class="form-label">Site Name</label><input type="text" name="site_name" value="{{ $settings['site_name'] ?? '' }}" class="form-input"></div>
                <div><label class="form-label">Tagline</label><input type="text" name="site_tagline" value="{{ $settings['site_tagline'] ?? '' }}" class="form-input"></div>
                <div><label class="form-label">Phone</label><input type="text" name="site_phone" value="{{ $settings['site_phone'] ?? '' }}" class="form-input"></div>
                <div><label class="form-label">Email</label><input type="email" name="site_email" value="{{ $settings['site_email'] ?? '' }}" class="form-input"></div>
                <div class="col-span-2"><label class="form-label">Address</label><input type="text" name="site_address" value="{{ $settings['site_address'] ?? '' }}" class="form-input"></div>
                <div>
    ...
    <div x-show="tab === 'orders'" class="bg-white rounded-xl border p-5">
        ...
        <form method="POST" action="{{ route('admin.settings.update') }}" class="space-y-4 mb-6">
            @csrf <input type="hidden" name="group" value="orders">
            <div class="grid grid-cols-2 gap-4">
                <div><label class="form-label">Default Delivery Charge (৳)</label><input type="number" name="delivery_charge" value="{{ $settings['delivery_charge'] ?? 60 }}" class="form-input"></div>
                <div><label class="form-label">Free Delivery Above (৳)</label><input type="number" name="free_delivery_min" value="{{ $settings['free_delivery_min'] ?? 500 }}" class="form-input"></div>
            </div>
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="guest_checkout" value="true" {{ ($settings['guest_checkout'] ?? '') === 'true' ? 'checked' : '' }} class="accent-teal-600">
        ...
        <div class="border-t pt-5">
            ...
            <div class="overflow-x-auto mb-4">
                <table class="admin-table">
                    <thead><tr><th>Zone</th><th>Division</th><th>Charge</th><th>Free Above</th><th>Days</th><th>Status</th><th>Actions</th></tr></thead>
                    <tbody>
                        @foreach($deliveryZones as $zone)
                        <tr x-data="{ editing: false }">
                            <td class="font-semibold text-sm">{{ $zone->name }}</td>
                            <td class="text-xs text-gray-500">{{ $zone->division }}</td>
                            <td>
                                <span x-show="!editing">৳{{ $zone->delivery_charge }}</span>
                                <form x-show="editing" method="POST" action="{{ route('admin.settings.delivery-zones.update', $zone) }}" class="flex gap-1 flex-wrap items-center">
                                    @csrf @method('PUT')
                                    <input type="hidden" name="name" value="{{ $zone->name }}">
                                    <input type="hidden" name="division" value="{{ $zone->division }}">
                                    <input type="number" name="delivery_charge" value="{{ $zone->delivery_charge }}" step="0.01" class="form-input py-1 text-xs" style="width:80px">
                                    <input type="number" name="free_delivery_above" value="{{ $zone->free_delivery_above }}" step="0.01" class="form-input py-1 text-xs" style="width:80px">
                                    <input type="number" name="estimated_days" value="{{ $zone->estimated_days }}" class="form-input py-1 text-xs" style="width:55px">
                            ...
                            <td x-show="!editing">৳{{ $zone->free_delivery_above }}</td>
                            <td x-show="!editing">{{ $zone->estimated_days }}d</td>
                            <td><span class="text-xs font-semibold {{ $zone->is_active ? 'text-green-600' : 'text-gray-400' }}">{{ $zone->is_active ? 'Active' : 'Off' }}</span></td>
                            <td x-show="!editing">
                                <div class="flex gap-1.5">
                                    <button type="button" @click="editing=true" class="btn-secondary btn-sm">Edit</button>
                                    <form method="POST" action="{{ route('admin.settings.delivery-zones.destroy', $zone) }}" onsubmit="return confirm('Delete this zone?')">
                                        @csrf @method('DELETE')
            ...
            <form method="POST" action="{{ route('admin.settings.delivery-zones.store') }}" class="grid grid-cols-3 gap-3">
                @csrf
                <input type="text" name="name" placeholder="Zone name (e.g. Dhaka City)" class="form-input" required>
                <select name="division" class="form-select" required>
                    <option value="">Division</option>
                    @foreach(config('bd.divisions') as $div)<option value="{{ $div }}">{{ $div }}</option>@endforeach
                </select>
                <input type="number" name="delivery_charge" placeholder="Charge (৳)" class="form-input" required>
                <input type="number" name="free_delivery_above" placeholder="Free above (৳)" value="500" class="form-input">
                <input type="number" name="estimated_days" placeholder="Est. days" value="2" class="form-input">
    ...
    <div x-show="tab === 'payment'" class="bg-white rounded-xl border p-5">
        ...
        <form method="POST" action="{{ route('admin.settings.update') }}" class="space-y-5">
            @csrf <input type="hidden" name="group" value="payment">

            {{-- Enable/Disable payment methods --}}
            <div class="space-y-3">
                <p class="text-sm font-semibold text-gray-700">Enable / Disable Methods</p>
                <p class="text-xs text-gray-400 -mt-1">bKash, Nagad, and Rocket are processed through SSL Commerz — enabling SSL Commerz shows all mobile banking options automatically.</p>
                @foreach([
                ...
                <div class="flex items-center justify-between p-3.5 border rounded-xl hover:bg-gray-50 transition-colors">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer flex-shrink-0 ml-3">
                        <input type="checkbox" name="{{ $key }}" value="true"
                            {{ ($settings[$key] ?? 'true') === 'true' ? 'checked' : '' }} class="sr-only peer">
            ...
            <div class="border-t pt-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div><label class="form-label">Store ID</label><input type="text" name="ssl_store_id" value="{{ $settings['ssl_store_id'] ?? '' }}" class="form-input"></div>
                    <div><label class="form-label">Store Password</label><input type="password" name="ssl_store_pass" value="{{ $settings['ssl_store_pass'] ?? '' }}" class="form-input"></div>
                </div>
                <label class="flex items-center gap-2 cursor-pointer mt-3">
                    <input type="checkbox" name="ssl_is_live" value="true" {{ ($settings['ssl_is_live'] ?? '') === 'true' ? 'checked' : '' }} class="accent-teal-600">
    ...
    <div x-show="tab === 'checkout'" class="bg-white rounded-xl border p-5">
        ...
        <p class="text-xs text-gray-500 mb-4">Control which fields appear on the checkout form. Required fields are always shown.</p>
        <form method="POST" action="{{ route('admin.settings.update') }}" class="space-y-3">
            @csrf <input type="hidden" name="group" value="checkout">
            @php
            $fields = [
            ...
            <div class="overflow-x-auto">
                <table class="admin-table">
                    <thead><tr><th>Field</th><th class="text-center">Visible</th><th class="text-center">Required</th></tr></thead>
                    <tbody>
                        {{-- Always-on fields (read only) --}}
                        @foreach([
                            'Full Name','Phone Number','Division','District','Full Address'
                        ] as $alwaysField)
                        <tr class="bg-gray-50">
                            <td class="text-sm font-medium text-gray-700">{{ $alwaysField }} <span class="text-xs text-teal-600">(always required)</span></td>
                        ...
                        @endphp
                        <tr>
                            <td class="text-sm font-medium text-gray-800">{{ $f['label'] }}</td>
                            <td class="text-center">
                                <input type="checkbox" name="checkout_field[{{ $f['key'] }}][visible]" value="1"
                                    {{ $visible ? 'checked' : '' }} class="accent-teal-600">
                            </td>
                            <td class="text-center">
                                <input type="checkbox" name="checkout_field[{{ $f['key'] }}][required]" value="1"
                                    {{ $required ? 'checked' : '' }} class="accent-teal-600">
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
    ...
    <div x-show="tab === 'sms'" class="bg-white rounded-xl border p-5">
        ...
        <form method="POST" action="{{ route('admin.settings.update') }}" class="space-y-4">
            @csrf <input type="hidden" name="group" value="sms">
            <div class="grid grid-cols-2 gap-4">
                <div><label class="form-label">API Key</label><input type="text" name="mimsms_api_key" value="{{ $settings['mimsms_api_key'] ?? '' }}" class="form-input" placeholder="From app.mimsms.com"></div>
                <div><label class="form-label">Sender ID</label><input type="text" name="mimsms_sender_id" value="{{ $settings['mimsms_sender_id'] ?? 'Ousodhaloy' }}" class="form-input"></div>
            </div>
            @foreach(['sms_order_confirm'=>'SMS on Order Placed','sms_status_update'=>'SMS on Status Update'] as $key=>$label)
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="{{ $key }}" value="true" {{ ($settings[$key] ?? '') === 'true' ? 'checked' : '' }} class="accent-teal-600">
                <span class="text-sm text-gray-700">{{ $label }}</span>
            </label>
            @endforeach
            <p class="text-xs text-gray-400">Get key: <a href="https://app.mimsms.com" target="_blank" class="text-teal-600 underline">app.mimsms.com</a></p>
    ...
    <div x-show="tab === 'pathao'" class="bg-white rounded-xl border p-5">
        ...
        <form method="POST" action="{{ route('admin.settings.update') }}" class="space-y-4">
            @csrf <input type="hidden" name="group" value="pathao">
            <div class="grid grid-cols-2 gap-4">
                <div><label class="form-label">Client ID</label><input type="text" name="pathao_client_id" value="{{ $settings['pathao_client_id'] ?? '' }}" class="form-input"></div>
                <div><label class="form-label">Client Secret</label><input type="password" name="pathao_client_secret" value="{{ $settings['pathao_client_secret'] ?? '' }}" class="form-input"></div>
                <div><label class="form-label">Username</label><input type="text" name="pathao_username" value="{{ $settings['pathao_username'] ?? '' }}" class="form-input"></div>
                <div><label class="form-label">Password</label><input type="password" name="pathao_password" value="{{ $settings['pathao_password'] ?? '' }}" class="form-input"></div>
                <div><label class="form-label">Store ID</label><input type="text" name="pathao_store_id" value="{{ $settings['pathao_store_id'] ?? '' }}" class="form-input"></div>
            </div>
            <label class="flex items-center gap-2 cursor-pointer">
            ...
            <div class="border-t pt-4">
                ...
                <p class="text-xs text-gray-500 mb-3">These will be pre-selected when pushing an order to Pathao. You can still change them per order.</p>
                <div class="grid grid-cols-3 gap-3">
                    <div>
                        <label class="form-label">Default City ID</label>
                        <input type="number" name="pathao_default_city_id"
                            value="{{ $settings['pathao_default_city_id'] ?? '' }}"
                            class="form-input" placeholder="e.g. 1 (Dhaka)">
                    ...
                    <div>
                        ...
                        <input type="number" name="pathao_default_zone_id"
                            value="{{ $settings['pathao_default_zone_id'] ?? '' }}"
                            class="form-input" placeholder="Zone ID">
                    </div>
                    <div>
                        <label class="form-label">Default Area ID</label>
                        <input type="number" name="pathao_default_area_id"
                            value="{{ $settings['pathao_default_area_id'] ?? '' }}"
    ...
    <div x-show="tab === 'steadfast'" class="space-y-4">
        <div class="bg-white rounded-xl border p-5">
            ...
            <form method="POST" action="{{ route('admin.settings.update') }}" class="space-y-4">
                @csrf <input type="hidden" name="group" value="steadfast">
                ...
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="steadfast_enabled" value="true"
                        {{ ($settings['steadfast_enabled'] ?? 'false') === 'true' ? 'checked' : '' }}
        ...
        {{-- Status card --}}
        @if(!empty($settings['steadfast_api_key']))
        <div class="bg-white rounded-xl border p-5">
            <h3 class="font-bold text-gray-800 mb-3 text-sm">Connection Status</h3>
            <div class="flex items-center gap-3">
                <div class="flex items-center gap-2 {{ ($settings['steadfast_enabled'] ?? '') === 'true' ? 'bg-green-50 border-green-200 text-green-700' : 'bg-gray-50 border-gray-200 text-gray-500' }} border rounded-xl px-4 py-2.5">
                    <span class="text-lg">{{ ($settings['steadfast_enabled'] ?? '') === 'true' ? '🟢' : '⚫' }}</span>
                    <div>
                        <p class="text-xs font-bold">Steadfast</p>
                        <p class="text-sm font-black">{{ ($settings['steadfast_enabled'] ?? '') === 'true' ? 'Enabled' : 'Disabled' }}</p>
                    </div>
                </div>
    ...
    <div x-show="tab === 'pixel'" class="space-y-4">
        ...
        <div class="bg-white rounded-xl border p-5">
            ...
            <p class="text-xs text-gray-500 mb-4">Connect your Facebook Pixel to track conversions. Purchase events fire automatically on order success.</p>

            <form method="POST" action="{{ route('admin.settings.pixel') }}" class="space-y-5">
                @csrf
                ...
                <div class="p-4 bg-gray-50 rounded-xl">
                    ...
                    <div class="flex gap-3">
                        @foreach(['live'=>['🟢 Live Mode','bg-green-50 border-green-300 text-green-800'],'test'=>['🧪 Test Mode','bg-yellow-50 border-yellow-300 text-yellow-800']] as $mode => [$mlabel, $mclass])
                        <label class="flex-1 cursor-pointer">
                            <input type="radio" name="meta_pixel_mode" value="{{ $mode }}" {{ ($settings['meta_pixel_mode'] ?? 'test') === $mode ? 'checked' : '' }} class="sr-only peer">
                            <div class="border-2 rounded-xl p-3 text-center text-sm font-bold transition-all peer-checked:border-teal-500 peer-checked:bg-teal-50 {{ $mclass }}">
                                {{ $mlabel }}
                            </div>
                        </label>
                        @endforeach
                    </div>
                    <div class="mt-3" id="test-code-wrap" x-data x-show="{{ ($settings['meta_pixel_mode'] ?? 'test') === 'test' ? 'true' : 'false' }}">
                        <label class="form-label">Test Event Code</label>
                        <input type="text" name="meta_pixel_test_event_code"
                            value="{{ $settings['meta_pixel_test_event_code'] ?? '' }}"
                            class="form-input font-mono" placeholder="TEST12345">
                        <p class="text-xs text-gray-400 mt-1">Get from Events Manager → Test Events → Enter code here</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="password" name="meta_access_token" value="{{ $settings['meta_access_token'] ?? '' }}"
                            class="form-input" placeholder="Optional — for server-side events">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        ] as $key => [$event, $desc, $default, $colors])
                        @php $enabled = ($settings[$key] ?? ($default ? 'true' : 'false')) === 'true'; @endphp
                        <div class="flex items-start justify-between p-3 border rounded-xl {{ $colors }}">
                            <div>
                                <p class="text-sm font-bold text-gray-800 font-mono">{{ $event }}</p>
                                <p class="text-xs text-gray-500 mt-0.5">{{ $desc }}</p>
                            </div>
                            <label class="relative inline-flex items-center cursor-pointer ml-2 flex-shrink-0">
                                <input type="checkbox" name="{{ $key }}" value="true" {{ $enabled ? 'checked' : '' }} class="sr-only peer">
        ...
        <div class="bg-white rounded-xl border p-5">
            ...
            <div class="flex flex-wrap gap-3">
                <div class="flex items-center gap-2 bg-{{ ($settings['meta_pixel_mode'] ?? '') === 'live' ? 'green' : 'yellow' }}-50 border border-{{ ($settings['meta_pixel_mode'] ?? '') === 'live' ? 'green' : 'yellow' }}-200 rounded-xl px-4 py-2.5">
                    <span class="text-lg">{{ ($settings['meta_pixel_mode'] ?? '') === 'live' ? '🟢' : '🧪' }}</span>
                    <div>
                ...
                <div class="flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-xl px-4 py-2.5">
                    ...
                    <div>
                        ...
                        <p class="text-sm font-mono font-black">{{ $settings['meta_pixel_id'] }}</p>
                    </div>
                </div>
                @if(!empty($settings['meta_pixel_test_event_code']) && ($settings['meta_pixel_mode'] ?? '') === 'test')
                <div class="flex items-center gap-2 bg-yellow-50 border border-yellow-200 rounded-xl px-4 py-2.5">
                    ...
                    <div>
                        ...
                        <p class="text-sm font-mono font-black">{{ $settings['meta_pixel_test_event_code'] }}</p>
                    </div>
                </div>
                @endif
                <a href="https://business.facebook.com/events_manager" target="_blank" class="flex items-center gap-2 bg-gray-50 border rounded-xl px-4 py-2.5 hover:bg-gray-100 transition-colors">
                    <i class="fas fa-external-link-alt text-gray-500 text-sm"></i>
